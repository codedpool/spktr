...
app.get('/preview', (req, res) => {
  if (!global.latestScreenshot) {
    return res.send('<body style="background:#111;color:white;padding:20px"><h2>No screenshot yet — wait 7 seconds</h2></body>')
  }
  res.send(`
    <html>
...
// WebSocket — receives screenshots from Tauri frontend
    ...
    try {
      ...
      if (data.type === 'screenshot') {
        ...
        if (global.latestScreenshot) {
          ...
          if (diff < 15) {
            console.log('[phash] Screen unchanged — skipping store')
            return
          }

          console.log(`[phash] Screen changed (${diff.toFixed(1)}%) — storing metadata`)

          // Store metadata in SQLite (NOT the raw image)
          db.prepare(`
        ...
        console.log(`[ws] Screenshot received — ${new Date().toISOString()}`)
      }
    } catch (e) {
